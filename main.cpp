...
addr_vector broadcast_addrs()
{
	...
	for (ifaddrs *ifa = ifap; ifa; ifa = ifa->ifa_next)
	{
				...
				(ifa->ifa_flags & IFF_BROADCAST) == IFF_BROADCAST)
		{
			...
			addrs.push_back(reinterpret_cast<sockaddr_in*>(ifa->ifa_broadaddr)
							->sin_addr.s_addr);
		}
	}
	freeifaddrs(ifap);
...
#define CLOSE_PIPESET(x) if (x != -1){close(x);x = -1;}
